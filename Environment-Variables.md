...
| `UPTIME_KUMA_ADMIN_PREFIX`                               |                               | Path prefix for all admin routes (dashboard, monitors, settings,  maintenance). Must start with / and must not end with / (e.g. `/admin`). When set, old unprefixed URLs (e.g. /dashboard) will automatically redirect to their prefixed equivalents. Public routes (status pages, setup) are never prefixed.                                                              |        `""` |

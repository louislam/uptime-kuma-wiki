...
> [!CAUTION]
> Exposing the Docker socket or TCP port to Uptime Kuma, gives that full control over the Docker daemon on the host. If the container is compromised, an attacker could gain control over it. It is NOT recommended to expose Uptime Kuma to the Internet when using this monitor type.
